    ...
    try:
        for episode in range(num_episodes):
            ...
            for step in range(max_steps_per_episode):
                ...
                # Previous code was calculating lateral_deviation manually using waypoints
                # Now, use the new function to get lane_center and lateral_offset
                lane_center, lateral_offset = env.get_lane_center_and_offset()
                if lane_center is None:
                    # If lane_center not found, skip this step
                    env.world.tick()
                    continue

                # For visualization or debugging, you can print or log the lane_center and lateral_offset
                # print(f"Lane Center: (x: {lane_center.x}, y: {lane_center.y}, z: {lane_center.z})")
                # print(f"Lateral Offset: {lateral_offset:.2f} meters")

                # **Use lateral_offset instead of lateral_deviation**
                # Update scalars accordingly
                # Assuming distance_to_destination remains the same
                ...
                if len(env.collision_sensor.get_history()) > 0:
                    ...
                    # **Modify Reward Calculation to Use lateral_offset**
                    ...
                    # Assuming epsilon was related to heading error; retain this calculation
                    # Calculate desired_yaw based on next waypoint
                    # For consistency, you may want to adjust this part as well if needed
                    # Here, we'll keep it as in the original code
                ...
                transition = Transition(
                    ...
                    n_scalars=scalars,  # You might want to update this with new scalars after action
                    d=done
                )
