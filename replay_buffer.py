...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size: int) -> Batch:
        ...
        # Convert to tensors
        img = torch.tensor(img_array, dtype=torch.float).unsqueeze(1).to(self.device)  # Shape: (batch_size, 1, H, W)
        ...
        n_img = torch.tensor(n_img_array, dtype=torch.float).unsqueeze(1).to(self.device)
